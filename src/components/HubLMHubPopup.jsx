  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '12px 14px 10px', background: '#FAFAF9', borderBottom: '1px solid #E8E8E8', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color, letterSpacing: '0.10em', textTransform: 'uppercase', marginBottom: 3 }}>
            {TIER_LABEL[tier]}
          </div>
          <div style={{ fontFamily: SERIF, fontSize: 14, color: '#111', lineHeight: 1.3, maxWidth: 180 }}>
            {hub.name || hub.labelBus || 'Hub'}
          </div>
          <div style={{ fontSize: 13, color: '#888', marginTop: 2 }}>{TIER_DESC[tier]}</div>
        </div>
      ...
      <div style={{ padding: '8px 14px', borderBottom: '1px solid #E8E8E8' }}>
        {hub.area > 0 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
            <span style={{ fontSize: 13, color: '#888' }}>Site area</span>
            <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 600, color: '#111' }}>{fmt(hub.area)}</span>
          </div>
        )}
        {hub.zone && (
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
            <span style={{ fontSize: 13, color: '#888' }}>Zone</span>
            <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 600, color: '#111', textTransform: 'capitalize' }}>{hub.zone}</span>
          </div>
        )}
        {hub.status && (
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
            <span style={{ fontSize: 13, color: '#888' }}>Status</span>
            <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 600, color: hub.status === 'existing' ? '#1D7A3A' : '#185FA5', textTransform: 'capitalize' }}>{hub.status}</span>
          </div>
        )}
        {hub.score !== undefined && (
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
            <span style={{ fontSize: 13, color: '#888' }}>Score</span>
            <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 600, color: '#111' }}>{Math.round(hub.score)}</span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '8px 14px 12px' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: '#888', letterSpacing: '0.10em', textTransform: 'uppercase', marginBottom: 6 }}>
        ...
        {fleetModes.map(([mode, n]) => {
          ...
          return (
            <div key={mode} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 0', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <span style={{ fontSize: 13, color: '#444', flex: 1 }}>{meta?.label || mode}</span>
              <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color }}>×{n}</span>
            </div>
          )
        })}
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6, paddingTop: 4, borderTop: `1px solid ${color}22` }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#222' }}>Total vehicles</span>
