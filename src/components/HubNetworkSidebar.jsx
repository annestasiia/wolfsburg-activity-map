...
function SectionLabel({ children }) {
  return (
    <div style={{ fontFamily: SANS, fontSize: 13, fontWeight: 700, color: C.text3, letterSpacing: '0.10em', textTransform: 'uppercase', margin: '20px 0 8px' }}>
      {children}
    </div>
  )
}
...
function StatRow({ label, value, unit, color }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '5px 0', borderBottom: `1px solid ${C.border}` }}>
      ...
      <span style={{ fontFamily: 'monospace', fontSize: 13, color: color || C.text1, fontWeight: 600 }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: `1px solid ${C.border}` }}>
      ...
      <input
        ...
        style={{ width: 80, padding: '3px 6px', border: `1px solid ${C.border}`, borderRadius: 4,
          fontFamily: 'monospace', fontSize: 13, color: C.text1, background: '#F7F7F7', textAlign: 'right' }}
      />
      <span style={{ fontFamily: SANS, fontSize: 13, color: C.text3, width: 20 }}>{unit}</span>
    </div>
  )
}
  ...
  return (
    <div style={{ border: `1px solid ${C.border}`, borderLeft: `3px solid ${color}`, borderRadius: 6, padding: '10px 12px', marginBottom: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontFamily: SANS, fontSize: 13, fontWeight: 700, color, letterSpacing: '0.06em' }}>{type}</span>
        <span style={{ fontFamily: SANS, fontSize: 13, color: C.text3 }}>{count} hubs · {candidateCount} candidates</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 4 }}>
        {[
          ...
          <div key={label} style={{ flex: 1, minWidth: 80, background: '#F7F7F7', borderRadius: 4, padding: '5px 8px' }}>
            ...
            <div style={{ fontFamily: 'monospace', fontSize: 13, color: C.text1, fontWeight: 600, marginTop: 2 }}>{value}</div>
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 6 }}>
        <div style={{ fontFamily: SANS, fontSize: 13, color: C.text2 }}>Centre: <strong>{centreCount}</strong> · {fmt(centreArea)}</div>
        <div style={{ fontFamily: SANS, fontSize: 13, color: C.text3 }}>·</div>
        <div style={{ fontFamily: SANS, fontSize: 13, color: C.text2 }}>Outer: <strong>{outerCount}</strong> · {fmt(outerArea)}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '18px 20px 14px', borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
        <div style={{ fontFamily: SANS, fontSize: 13, fontWeight: 700, color: C.text3, letterSpacing: '0.10em', textTransform: 'uppercase', marginBottom: 6 }}>
          Hub Network
        </div>
      ...
      <div style={{ flex: 1, padding: '0 16px 24px', overflowY: 'auto' }}>
        ...
        <div style={{ margin: '14px 0 4px' }}>
          ...
          {hubLMRunning && (
            <div style={{ fontFamily: SANS, fontSize: 13, color: C.text3, marginTop: 6, textAlign: 'center' }}>
              {hubLMStatus}
            </div>
          )}
          {!ready && !hubLMRunning && (
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', padding: '8px 0', cursor: 'pointer', fontFamily: SANS, fontSize: 13, color: C.text3, display: 'flex', alignItems: 'center', gap: 4 }}
        >
        ...
        {hubLMResults ? (
          <>
            ...
            <div style={{ border: `1px solid ${C.border}`, borderLeft: `3px solid ${C.hubS}`, borderRadius: 6, padding: '10px 12px', marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontFamily: SANS, fontSize: 13, fontWeight: 700, color: C.hubS, letterSpacing: '0.06em' }}>HUB S</span>
                <span style={{ fontFamily: SANS, fontSize: 13, color: C.text3 }}>{hubS.length} hubs selected</span>
              </div>
